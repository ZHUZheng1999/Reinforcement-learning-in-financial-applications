...
class Binomial_stock(object):
    ...
    def __init__(self, S0, K, rf, sigma, n_step):
        ...
        # self.early_excecise = np.zeros((2*self.n_step+1, self.n_step+1))
...
#                 option_payoff = np.maximum(self.K - self.stock_price[i, j], 0)
                self.option_price[i, j] = option_price
#                 if option_payoff >= option_price:
#                     self.option_price[i, j] = option_payoff
#                     self.early_excecise[i, j] = True
#                 else:
#                     self.option_price[i, j] = option_price
    ...
    def step(self, action, state, already_strike):
        ...
        if proba == 0:
            ...
            next_option_price = self.option_price[pos+1, int(state[0])+1]
...
#         next_option_price = self.option_price[next_pos, int(next_state[0])]
        ...
        if already_strike:
            reward = action * (next_stock_price - this_stock_price)
        else:
#             already_strike = self.early_excecise[pos, int(state[0])]
            this_option_payoff = np.maximum(self.K - this_stock_price, 0)
            if this_option_payoff > this_option_price:
                already_strike = True
                reward = action * (next_stock_price - this_stock_price)
            else:
...
#                 reward = next_option_price - this_option_price + action * (next_stock_price - this_stock_price)
        ...
        return next_state, reward, done, already_strike
...
class GBM_stock(object):
    ...
    def step(self, action, already_strike):
        this_option = self.put_option_price(self.S, self.K, self.rf, self.sigma, self.T)
        this_stock = self.S
        # delta = self.action_map[int(action)]
        # delta = action
        self.T -= self.dt
        self.S = self.S * np.exp((self.rf - self.sigma ** 2 / 2) * self.dt + self.sigma * np.random.normal() * np.sqrt(self.dt))
        next_option = self.put_option_price(self.S, self.K, self.rf, self.sigma, self.T)

        if already_strike:
            reward = action * (self.S - this_stock)
        else:
            this_option_payoff = np.maximum(self.K - this_stock, 0)
            if this_option_payoff > this_option:
                already_strike = True
                reward = action * (self.S - this_stock) # + this_option_payoff
            else:
                next_option = self.put_option_price(self.S, self.K, self.rf, self.sigma, self.T)
                reward = next_option - this_option + action * (self.S - this_stock)
...
# Binomial_stock = Binomial_stock(S0=40, K=50, rf=0.05, sigma=0.05, n_step=5)
# print(Binomial_stock.stock_price)
# print(Binomial_stock.option_price)
# print(Binomial_stock.early_excecise)
# GBM_stock = GBM_stock(S0=50, K=50, rf=0.05, sigma=0.05, n_step=5)
# print(GBM_stock.stock_price)
# print(GBM_stock.option_price)
# print(GBM_stock.early_excecise)
